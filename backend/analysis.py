...
class StudentAnalyzer:
    ...
    def _build_workbook(self) -> Workbook:
        """Build and return the openpyxl Workbook object."""
        ...
        for label, value in topper_metrics:
            ws1[f"A{row}"] = label
            ws1[f"B{row}"] = value
            for col in ("A", "B"):
        ...
        topper_row_fill = PatternFill("solid", fgColor="FFF9C4")
        for i, student in enumerate(analysis["ranked_students"]):
            ...
            row_fill  = topper_row_fill if is_topper else (highlight_fill if i % 2 == 0 else white_fill)
    ...
    def generate_excel_report_bytes(self) -> bytes:
        """Return the Excel report as raw bytes (for streaming responses / serverless)."""
